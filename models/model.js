                ...
                Tinynews.odb.query(
                    ...
                    function(response) {
                        ...
                        if (response && response['@rid']) {
                            that['@rid'] = Tinynews.RID.Build(response['@rid']);
            ...
            if (i != '@rid' && i != 'slug') {
                if (this[i] != undefined) {
                    switch (this.properties[i].type) {
                        ...
                        case 'long':
                        case 'decimal':
                            field_values += i + "=" + Tinynews.escape(this[i]) + ",";
                            break;
        ...
        for (var i in options) {
            if (options[i] instanceof Array) {
                ...
                for (var j in options[i]) {
                    result += Tinynews.escape(options[i][j]) + ",";
                }
                ...
                // --
                result += i.toString() + "='" + Tinynews.escape(options[i]) + "'" + mode;
            }
        }
        result = result.substr(0, result.length - mode.length);
